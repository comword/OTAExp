...
int32_t  bmp280CompensatePressure(int32_t adc_P)
{  
  int64_t var1, var2;
  uint64_t pressure;  
...
void bmp280TandPGet(float *temperature, int32_t *pressure)  
{  
...
void pressSensorDataGet(int32_t *ps32Temperature, int32_t *ps32Pressure, int32_t *ps32Altitude)
{
  ...
  int32_t CurAltitude, CurPressure;
  static BMP280_AvgTypeDef BMP280_Filter[3];

  bmp280TandPGet(&CurTemperature, &CurPressure);
  bmp280CalAvgValue(&BMP280_Filter[0].Index, BMP280_Filter[0].AvgBuffer, (CurPressure), ps32Pressure);
